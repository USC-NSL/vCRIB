 ...
 */
public class VMStartMigrateRandomProcessor extends AbstractMigrator implements InformOnRestart {
    ...
    public VMStartMigrateRandomProcessor(Topology topology, Random random, int threadNum, Map<Partition, Map<Rule, Collection<Flow>>> ruleFlowMap,
    ...
    public Map<Partition, Switch> postPlace2(Set<Switch> availableSwitches, Assignment assignment, PrintWriter trendWriter) {
...

        {
            ...
            for (Partition partition : partitions) {
                currentTrafficSum += topology.getTrafficForHosting(partition, assignment.getPlacement().get(partition));
            }

            double currentResourceUsage = initCurrentResourceUsage();
